...
public class EmpresaController {
    ...
    // Método para validar las credenciales del usuario y redirigir a la página de inicio
    ...
    public String iniciarSesionemp(Model model, @RequestParam String email, @RequestParam String contraseña) {
        ...
        Empresas empresa = u_emp.findByEmailAndContraseña(email, contraseña); // Reemplazamos Usuario_empresa con Empresas

        // Validar si la empresa existe
        if (empresa != null) {
            model.addAttribute("nombreEmpresa", empresa.getNombreEmp());
            return "html/InicioEmp"; // Redirigir a la página de inicio de empresa
        } else {
            model.addAttribute("error", "Credenciales incorrectas o empresa no encontrada");
            return "html/contraseña_incorrectaemp"; // Vista de error
        }
    }

	@GetMapping("/inicio_empresa") //inicio de empresa, como lo verian las empresas 
    ...
    @GetMapping("/oferta") //para ver las ofertas postuladas 
