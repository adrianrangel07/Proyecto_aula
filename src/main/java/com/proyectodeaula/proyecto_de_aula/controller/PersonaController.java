...
public class PersonaController {
    ...
    public String iniciarSesion(Model model, @RequestParam String email, @RequestParam String contraseña) {
        ...
        Personas persona = user.findByEmailAndContraseña(email, contraseña); //metodo de busqueda de persona (correo y contraseña)

        // Validar si la persona existe
        if (persona != null) {
            model.addAttribute("nombreUsuario", persona.getnombre()); //si es correcto 
            return "redirect:/login/personas"; // Redirigir a la ruta de inicio de persona registrada 
        } else {
